...
Table~\ref{tab:bench-results} consolidates the benchmark defined in Section~5. The ten warm repetitions were remarkably stable, with a coefficient of variation below 1\% in end-to-end latency, which corresponds to 28.6 input words per second, or approximately 35~s per 1,000 words, at the \texttt{medium} level.
...
Taken together, these findings suggest that the value of LASCAS lies less in competing with model-centered approaches and more in showing how simplification, accessibility, and real use can be articulated within the same artifact.
...
This applied positioning is reinforced by the methodological stance of the work: the use of real documents, distinct participant profiles, and local execution conditions brought the evaluation closer to realistic contexts of use. The local-first design also made visible the practical trade-off between privacy, feasibility, latency, and model capacity, which is central for legal documents that may contain sensitive information. The backend benchmark made this trade-off concrete: on a consumer laptop GPU, a full petition was simplified in about two and a half minutes with the accelerator nearly saturated, showing that model inference defines the performance boundary of the approach.

The study therefore points to a design direction in which legal simplification tools should be assessed not only by whether they produce simpler sentences, but also by whether they help users approach documents with more orientation, confidence, and control. Looking ahead, LASCAS opens directions for broader evaluations with objective comprehension tasks, richer glossary support, stronger PDF handling, OCR integration, latency optimizations for long documents, and models better adapted to Brazilian legal Portuguese. Its main contribution is not a claim of full automation, but a practical demonstration that legal AI can be designed as a careful layer of support for comprehension, without depending on excessively complex infrastructure. By keeping the system bounded to initial reading and triage, the work preserves the distinction between accessibility support and legal interpretation, and allows future improvements in model quality, document coverage, and evaluation methods without changing the central role of the tool: helping users approach legal information with less friction and more autonomy.
